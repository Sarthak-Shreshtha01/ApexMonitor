    ...
    <div className="bg-surface-container-low p-8 rounded-2xl border border-outline-variant/10 shadow-[0_20px_40px_-12px_rgba(0,0,0,0.5)] mb-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-10">
        ...
        <div className="flex gap-2">
          ...
          <button className="bg-primary/10 border border-primary/20 px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-widest text-primary flex items-center gap-2 hover:bg-primary/20 transition-colors">
      ...
      <div className="h-[320px] w-full relative">
        ...
        <div className="absolute inset-0 flex flex-col justify-between pointer-events-none">
          {[1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="border-t border-outline-variant/50 w-full h-px"></div>
          ))}
        </div>
        ...
        <svg className="w-full h-full relative z-10" preserveAspectRatio="none" viewBox="0 0 1000 300">
          ...
          <path className="drop-shadow-[0_0_8px_rgba(255,69,0,0.4)]" d="M0,250 Q100,220 200,240 T400,180 T600,200 T800,120 T1000,150" fill="none" stroke="#ff4500" strokeWidth="2"></path>
        </svg>

        {/* Custom Crosshair Tooltip (Mockup) */}
        <div className="absolute left-1/2 top-0 bottom-0 w-px bg-primary/40 pointer-events-none z-20">
          <div className="absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 w-3 h-3 bg-primary rounded-full border-2 border-app ring-4 ring-primary/20"></div>
          <div className="absolute top-1/3 left-4 bg-surface-container-highest/80 backdrop-blur-md p-3 rounded-lg border border-outline-variant/20 shadow-2xl min-w-[120px]">
            <p className="text-[10px] text-secondary uppercase font-bold tracking-tighter mb-1">14:42:00</p>
      ...
      <div className="flex justify-between mt-4">
        {['12:00', '15:00', '18:00', '21:00', '00:00'].map((time) => (
          <span key={time} className="text-[10px] font-bold text-muted">{time}</span>
        ))}
      </div>
    </div>
  );
