...
interface TraceDetailPageClientProps {
  traceId: string;
}

export function TraceDetailPageClient({ traceId }: TraceDetailPageClientProps) {
  const activeProjectId = useProjectStore((state) => state.activeProjectId);

  const traceQuery = useQuery({
    queryKey: ['traceDetail', traceId, activeProjectId],
    queryFn: () => tracesService.detail(traceId, activeProjectId as string),
