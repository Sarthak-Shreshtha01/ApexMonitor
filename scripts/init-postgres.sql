...
-- Table 2b: rum_public_keys
CREATE TABLE IF NOT EXISTS rum_public_keys (
    id              BIGSERIAL    PRIMARY KEY,
    project_id      VARCHAR(64)  NOT NULL REFERENCES projects(id) ON DELETE CASCADE,
    key_prefix      VARCHAR(10)  NOT NULL,
    key_hash        VARCHAR(255) NOT NULL UNIQUE,
...
CREATE INDEX IF NOT EXISTS idx_rum_keys_prefix ON rum_public_keys (key_prefix);
CREATE INDEX IF NOT EXISTS idx_rum_keys_project ON rum_public_keys (project_id);

-- Table 2c: hourly_rum_metrics
...
CREATE INDEX IF NOT EXISTS idx_hrm_project_time ON hourly_rum_metrics (project_id, hour_bucket DESC);
CREATE INDEX IF NOT EXISTS idx_hrm_project_path_time ON hourly_rum_metrics (project_id, path, hour_bucket DESC);
CREATE INDEX IF NOT EXISTS idx_hrm_project_device_time ON hourly_rum_metrics (project_id, device_type, browser_name, hour_bucket DESC);
